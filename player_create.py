    ...
    class Player:
        def Create(self, i):
            ...
            if "Weapon 2" in data[i]:
                ...
                PValues={'PName:':PlayerName,'Class:':Pclass,'BaseHP:':HP, 'CurrentHP:':HP,'ModifiedHP:':HP, 'Weapon1:':Weapon1,'Moves1:':Moves1,'Weapon2:':Weapon2, 'Moves2':Moves2}
            else:
                PValues={'PName:':PlayerName,'Class:':Pclass,'BaseHP:':HP, 'CurrentHP:':HP,'ModifiedHP:':HP,'Weapon1:':Weapon1,'Moves1:':Moves1}
            return(PValues)
        def Create2(self, x):
            with open("player.json", "r") as pjson:
                data = json.load(pjson)
